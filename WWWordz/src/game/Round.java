...
extends java.lang.Object   {
	...
	/**puzzle nao esta a reconhecer **/
	public Puzzle getPuzzle() throws WWWordzException, IOException {
		puzzle = Generator.generate();
		Date cur = new Date();
		if(cur.before(play)){
			throw new WWWordzException("Exception expected before play stage");
		}
	...
	public List<Rank> getRanking() throws WWWordzException {
		Date cur =new Date();
		if(cur.before(ranking)) throw new WWWordzException("Exception expected in  stage");
		
		ArrayList<>
		/** usar a hash map para comparar os players e retornar **/ 
	...
	public long getTimetoNextPlay() {
		Date cur = new Date();
		if(cur.before(play)) return play.getTime()-cur.getTime();
		else
	...
	public long register(String nick, String password) throws WWWordzException {
		...
		if(cur.after(play)) throw new WWWordzException("Set points at different stages");
		else if(!theplayers.verify(nick,password)) throw new WWWordzException("Invalid User");
	...
	public void setPoints(String nick, int points) throws WWWordzException {
		Date cur = new Date();
		if(cur.before(report)) throw new WWWordzException("Set points at different stages");
		else if(cur.after(ranking)) throw new WWWordzException("Exception expected in rankings stage");
